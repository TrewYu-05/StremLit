...
def get_llm_response(client, user_prompt, stream=True):
    response = client.chat.completions.create(
        model="qwen-plus", # Use qwen model compatible with base_url
        messages=[
...
def generate_summary(url):
    # Retrieve api key directly from env to pass to openai client
...
st.set_page_config(page_title="网页摘要生成助手 (OpenAI SDK)", page_icon="📝", layout="wide")

st.write('## 网页摘要生成助手 (OpenAI SDK版)')
st.divider()
